            ...
            try
            {
                ...
                using (var connection = _context.Database.GetDbConnection())
                {
                    ...
                    using (var command = connection.CreateCommand())
                    {
                        ...
                        command.CommandText = "EXEC SP_XuLyGioHang @MAND, @PhuongThucThanhToan, @NgayThanhToan";
                        command.Parameters.Add(new SqlParameter("@MAND", input.MAND));
                        command.Parameters.Add(new SqlParameter("@PhuongThucThanhToan", input.PhuongThucThanhToan));
                        command.Parameters.Add(new SqlParameter("@NgayThanhToan", DateTime.Now));

                        // Thực thi Stored Procedure và đọc kết quả trả về
                        using (var reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                // Đọc kết quả từ Stored Procedure
                                var mahd = reader["MAHD"].ToString();
                                var macthdList = reader["MACTHD_LIST"].ToString();
                ...
                return StatusCode(500, new { message = "Không thể xử lý kết quả từ Stored Procedure." });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Đã xảy ra lỗi khi thực hiện thanh toán: " + ex.Message });
